...
static ElementScene *sceneCreerElementScene(const Scene *scene, int type)
{
    ...
    switch (type) {
        case ELEMENT_TYPE_DEBRIS_ASTEROIDE:
            elementInit(e, ELEMENT_TYPE_DEBRIS_ASTEROIDE, RESS_IMG_DEBRIS_ASTEROIDE,
                        ressourceGetLargeurImage(scene->ressource, RESS_IMG_DEBRIS_ASTEROIDE),
                        ressourceGetHauteurImage(scene->ressource, RESS_IMG_DEBRIS_ASTEROIDE), scene->largeurAffichage,
                        scene->hauteurAffichage);
            break;
        case ELEMENT_TYPE_BONUS_SCORE:
            elementInit(e, ELEMENT_TYPE_BONUS_SCORE, RESS_IMG_BONUS_SCORE,
                        ressourceGetLargeurImage(scene->ressource, RESS_IMG_BONUS_SCORE),
                        ressourceGetHauteurImage(scene->ressource, RESS_IMG_BONUS_SCORE), scene->largeurAffichage,
                        scene->hauteurAffichage);
            break;
        case ELEMENT_TYPE_BONUS_MISSILE:
            elementInit(e, ELEMENT_TYPE_BONUS_MISSILE, RESS_IMG_BONUS_MISSILE,
                        ressourceGetLargeurImage(scene->ressource, RESS_IMG_BONUS_MISSILE),
                        ressourceGetHauteurImage(scene->ressource, RESS_IMG_BONUS_MISSILE), scene->largeurAffichage,
                        scene->hauteurAffichage);
            break;
        case ELEMENT_TYPE_CROISEUR:
            elementInit(e, ELEMENT_TYPE_CROISEUR, RESS_IMG_VAISSEAU_CROISEUR,
                        ressourceGetLargeurImage(scene->ressource, RESS_IMG_VAISSEAU_CROISEUR),
                        ressourceGetHauteurImage(scene->ressource, RESS_IMG_VAISSEAU_CROISEUR), scene->largeurAffichage,
                        scene->hauteurAffichage);
            break;
        case ELEMENT_TYPE_VAISSEAU_JOUEUR:
            elementInit(e, ELEMENT_TYPE_VAISSEAU_JOUEUR, RESS_IMG_VAISSEAU_JOUEUR,
                        ressourceGetLargeurImage(scene->ressource, RESS_IMG_VAISSEAU_JOUEUR),
                        ressourceGetHauteurImage(scene->ressource, RESS_IMG_VAISSEAU_JOUEUR),
                        scene->largeurAffichage, scene->hauteurAffichage);
            break;
        case ELEMENT_TYPE_ECLAIREUR:
            elementInit(e, ELEMENT_TYPE_ECLAIREUR, RESS_IMG_VAISSEAU_ECLAIREUR,
                        ressourceGetLargeurImage(scene->ressource, RESS_IMG_VAISSEAU_ECLAIREUR),
                        ressourceGetHauteurImage(scene->ressource, RESS_IMG_VAISSEAU_ECLAIREUR),
                        scene->largeurAffichage, scene->hauteurAffichage);
            break;
        case ELEMENT_TYPE_ASTEROIDE:
            elementInit(e, ELEMENT_TYPE_ASTEROIDE, RESS_IMG_ASTEROIDE,
                        ressourceGetLargeurImage(scene->ressource, RESS_IMG_ASTEROIDE),
                        ressourceGetHauteurImage(scene->ressource, RESS_IMG_ASTEROIDE), scene->largeurAffichage,
                        scene->hauteurAffichage);
            break;
        case ELEMENT_TYPE_CHASSEUR:
            elementInit(e, ELEMENT_TYPE_CHASSEUR, RESS_IMG_VAISSEAU_CHASSEUR,
                        ressourceGetLargeurImage(scene->ressource, RESS_IMG_VAISSEAU_CHASSEUR),
                        ressourceGetHauteurImage(scene->ressource, RESS_IMG_VAISSEAU_CHASSEUR), scene->largeurAffichage,
                        scene->hauteurAffichage);
            break;
        case ELEMENT_TYPE_LASER_ENNEMI:
            elementInit(e, ELEMENT_TYPE_LASER_ENNEMI, RESS_IMG_TIR_ENNEMI_LASER,
                        ressourceGetLargeurImage(scene->ressource, RESS_IMG_TIR_ENNEMI_LASER),
                        ressourceGetHauteurImage(scene->ressource, RESS_IMG_TIR_ENNEMI_LASER), scene->largeurAffichage,
                        scene->hauteurAffichage);
            break;
        case ELEMENT_TYPE_MISSILE_ENNEMI:
            elementInit(e, ELEMENT_TYPE_MISSILE_ENNEMI, RESS_IMG_TIR_ENNEMI_MISSILE,
                        ressourceGetLargeurImage(scene->ressource, RESS_IMG_TIR_ENNEMI_MISSILE),
                        ressourceGetHauteurImage(scene->ressource, RESS_IMG_TIR_ENNEMI_MISSILE),
                        scene->largeurAffichage, scene->hauteurAffichage);
            break;
        case ELEMENT_TYPE_LASER_JOUEUR :
            elementInit(e, ELEMENT_TYPE_LASER_JOUEUR, RESS_IMG_TIR_JOUEUR_LASER,
                        ressourceGetLargeurImage(scene->ressource, RESS_IMG_TIR_JOUEUR_LASER),
                        ressourceGetHauteurImage(scene->ressource, RESS_IMG_TIR_JOUEUR_LASER), scene->largeurAffichage,
                        scene->hauteurAffichage);
            break;
        case ELEMENT_TYPE_MISSILE_JOUEUR:
            elementInit(e, ELEMENT_TYPE_MISSILE_JOUEUR, RESS_IMG_MISSILE_JOUEUR,
                        ressourceGetLargeurImage(scene->ressource, RESS_IMG_MISSILE_JOUEUR),
                        ressourceGetHauteurImage(scene->ressource, RESS_IMG_MISSILE_JOUEUR), scene->largeurAffichage,
                        scene->hauteurAffichage);
            break;
        default :
            break;
...
void sceneInit(Scene *scene, Ressource *res, Joueur *player, int largeurGraphique, int hauteurGraphique)
{
    ...
    assert(scene->acteurs.tab != NULL && scene->tirs.tab != NULL && scene->bonus.tab != NULL &&
           scene->decors.tab != NULL && scene->positionsExplosions.tab != NULL);
    ...
    elementSetPosition(scene->elementVaisseauJoueur, 32,
                       (hauteurGraphique - scene->elementVaisseauJoueur->hauteur) / 2);
    ...
    for (i = 0; i < sceneGetNbTirs(scene); i++) {
        ...
        switch (typeElement) {
            ...
            case ELEMENT_TYPE_MISSILE_JOUEUR:
                ...
                break;
            case ELEMENT_TYPE_LASER_ENNEMI:
                dx = -(int) (dt * SCENE_VITESSE_TIR * 3.0f /
                             4.0f);/* (note:le facteur 3/4 simule la vitesse moindre des lasers ennemis par rapport au laser du joueur) */
                break;
            case ELEMENT_TYPE_MISSILE_ENNEMI:
        ...
        switch (elementGetType(t)) {
            ...
            case ELEMENT_TYPE_LASER_JOUEUR:
                ...
                while (j < sceneGetNbActeurs(scene) && collision == 0) {
                    ...
                    else if (typeElement == ELEMENT_TYPE_ECLAIREUR || typeElement == ELEMENT_TYPE_CHASSEUR ||
...
void sceneEnnemiDeclencheTir(Scene *scene, ElementScene *e, float tempsCourant)
{
    ...
    if (tempsCourant - arme->tempsDernierTir - 0.25f * randomFloat() < modCadence * arme->cadence ||
        vaisseauGetMunitionsArme(vaisseau) <= 0)
        return;
